"""``webcam-cli cli`` — noun grouping CLI-surface introspection.
...
def _no_verb(args: argparse.Namespace) -> int:
    # `webcam-cli cli` with no sub-verb prints the noun's overview.
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "cli",
        help="CLI-surface introspection (see 'webcam-cli cli overview').",
    )
    ...
    ov = noun_sub.add_parser("overview", help="Describe the webcam-cli CLI surface.")
