"""``webcam cli`` — noun grouping CLI-surface introspection.
...
(distinct from the global ``overview``, which describes the agent, and from
``stream overview``, which describes the capture noun).
"""
...
from webcam_cli.cli._commands.overview import cli_sections, emit_overview
...
def _no_verb(args: argparse.Namespace) -> int:
    # `webcam cli` with no sub-verb prints the noun's overview.
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "cli",
        help="CLI-surface introspection (see 'webcam cli overview').",
    )
    ...
    ov = noun_sub.add_parser("overview", help="Describe this CLI's own surface.")
