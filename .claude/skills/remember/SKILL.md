...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope webcam-cli
...
scope:** the default personal scope is **private** (`--scope webcam-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `webcam-cli`). Private records are served only to a recall in the same
