"""Smoke tests for the webcam-cli CLI entry point and its verbs."""
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: webcam-cli" in capsys.readouterr().out
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "webcam-cli whoami" in payload["markdown"]
